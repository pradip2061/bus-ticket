...
export default function BookingSeat() {
  ...
  const handleSelect = (seatNumber) => {
    if (seatBook.includes(seatNumber)) {
      ...
      if (seatBook.length < 6) {
        ...
        toast.warn("Maximum 6 seats can be selected");
      }
    }
  };
  ...
  useEffect(() => {
    const getSignature = async () => {
      ...
      try {
        ...
        setformData((prev) => ({
          ...prev,
          signature: res.data.hashedSignature,
        }));
      } catch (error) {
  ...
  const handleSubmit = (e) => {
    ...
    if (parseInt(formData.amount) !== totalPrice) {
      toast.error(`Amount must match total price: Rs.${totalPrice}`);
      return;
    }

    // Check signature
    if (!formData.signature) {
      toast.error("Signature not generated yet. Please wait.");
      return;
    }
                ...
                {path?.seats
                  ...
                  .map((seat) => (
                    <button
                      ...
                      className={`w-14 h-14 rounded border flex justify-center items-center text-sm font-medium ${
                        seat.isBooked
        ...
        <div className="bg-white rounded-xl shadow p-4 sm:p-6 sticky top-6">
          ...
          >
            ...
            <input
              type="number"
              name="amount"
              className="border pl-2 w-full rounded-md p-2"
